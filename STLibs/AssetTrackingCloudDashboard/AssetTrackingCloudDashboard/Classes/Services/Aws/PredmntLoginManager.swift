...
public class PredmntLoginManager {
    public static let shared = PredmntLoginManager()
    
    private var keychain: Keychain
    private var keychainKey = "com.st.clab.STPredictiveMaintenance.authKey"
    // AppAuth
...
extension PredmntLoginManager: LoginManager {
    ...
    public func authenticate(from controller: UIViewController, completion: @escaping AuthCompletion) {
        ...
        self.currentAuthorizationFlow = OIDAuthState.authState(byPresenting: request, presenting: controller) { [weak self] authState, error in
            ...
            print("Got PREDMNT authorization tokens. Access token: " +
    ...
    public func buildDeviceManager(completion: @escaping (Result<DeviceManager, DeviceOperationError>) -> Void) {
        ...
        guard let accessKey = authState?.lastTokenResponse?.accessToken else { return completion(.failure(.missingToken)) }
        
        UserDefaults.standard.set(idToken, forKey: "PMidTokenN")
        UserDefaults.standard.set(accessKey, forKey: "PMaccessTokenN")
        
        let credential = AuthzCredential(accessKeyId: accessKey, secretKey: " ", sessionToken: idToken)
        print("PREDICTIVE MAINTENANCE TOKEN")
        print(credential)
    ...
    public func checkLA(idTokenN: String, accessTokenN: String){
        ...
        if(showLAPM){
            UserDefaults.standard.set(false, forKey: "PMshowLA")
        }else{
            UserDefaults.standard.set(true, forKey: "PMshowLA")
        }
    }
...
private extension PredmntLoginManager {
    func saveState() {
