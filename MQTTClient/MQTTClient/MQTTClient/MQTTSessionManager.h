 ...
 */
@protocol MCMQTTSessionManagerDelegate <NSObject>

/**
 Enumeration of MCMQTTSessionManagerState values
 */
typedef NS_ENUM(int, MCMQTTSessionManagerState) {
    MCMQTTSessionManagerStateStarting,
    MCMQTTSessionManagerStateConnecting,
    MCMQTTSessionManagerStateError,
    MCMQTTSessionManagerStateConnected,
    MCMQTTSessionManagerStateClosing,
    MCMQTTSessionManagerStateClosed
};
...
/** gets called when a new message was received
 @param sessionManager the instance of MCMQTTSessionManager whose state changed
 @param data the data received, might be zero length
 @param topic the topic the data was published to
 @param retained indicates if the data retransmitted from server storage
 */
 ...
 */
- (void)sessionManager:(MCMQTTSessionManager *)sessionManager didChangeState:(MCMQTTSessionManagerState)newState;

@end

/** SessionManager handles the MQTT session for your application
 */
@interface MCMQTTSessionManager : NSObject <MCMQTTSessionDelegate>

/** Underlying MCMQTTSession currently in use.
 */
@property (strong, nonatomic, readonly) MCMQTTSession *session;
 ...
 */
@property (weak, nonatomic) id<MCMQTTSessionManagerDelegate> delegate;
 ...
 */
@property (nonatomic, readonly) MCMQTTSessionManagerState state;

/** SessionManager last error code when state equals MCMQTTSessionManagerStateError
 */
 ...
 * @param maxRetryInterval The duration at which the connection-retry timer should be capped. When MCMQTTSessionManager receives a ClosedByBroker or an Error
 ...
 * @param connectInForeground Whether or not to connect the MCMQTTSession when the app enters the foreground, and disconnect when it becomes inactive. When NO, the caller is responsible for calling -connectTo: and -disconnect. Defaults to YES.
 * @return the initialized MCMQTTSessionManager object
 */
 ...
 * @param runLoop Run loop for MCMQTTSession.
 */
