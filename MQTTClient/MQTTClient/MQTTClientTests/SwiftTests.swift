...
class SwiftTests : MQTTTestHelpers {
    ...
    override func handleEvent(_ session: MCMQTTSession!, event eventCode: MCMQTTSessionEvent, error: Error!) {
        switch eventCode {
        case .connected:
    ...
    override func newMessage(_ session: MCMQTTSession!, data: Data!, onTopic topic: String!, qos: MQTTQosLevel, retained: Bool, mid: UInt32) {
        print("Received \(data) on:\(topic) q\(qos) r\(retained) m\(mid)")
        sessionReceived = true;
    }
    
    override func subAckReceived(_ session: MCMQTTSession!, msgID: UInt16, grantedQoss qoss: [NSNumber]!) {
