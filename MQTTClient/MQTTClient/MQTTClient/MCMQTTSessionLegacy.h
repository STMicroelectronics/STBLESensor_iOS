...
@interface MCMQTTSession(Create)
 ...
 */
- (MCMQTTSession *)initWithClientId:(NSString *)clientId
 ...
 * @code
 ...
 NSString* certificate = [[NSBundle bundleForClass:[MCMQTTSession class]] pathForResource:@"certificate" ofType:@"cer"];
 ...
 MCMQTTSession *session = [[MCMQTTSession alloc]
...
/** for mqttio-OBJC backward compatibility
 @param theClientId see initWithClientId for description.
 @param theKeepAliveInterval see initWithClientId for description.
 @param theConnectMessage has to be constructed using MCMQTTMessage connectMessage...
 @param queue see initWithClientId for description.
 @return the initialised MQTTSession object
 All other parameters are set to defaults
 */
...
- (void)connectToHost:(NSString *)ip
                 port:(UInt32)port
withConnectionHandler:(void (^)(MCMQTTSessionEvent event))connHandler
