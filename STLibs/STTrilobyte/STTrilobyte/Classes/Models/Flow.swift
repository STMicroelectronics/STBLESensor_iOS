...
extension Flows: Uploadable where Element: Flow {
    func data() -> Data? {
        ...
        guard let flowData = try? JSONSerialization.data(withJSONObject: jsonFlows, options: []) else {
            return nil
        }
...
extension Flow {
    ...
    func availableOutputs() -> [Output] {
        ...
        let allOutputs = PersistanceService.shared.getAllOutputs()
