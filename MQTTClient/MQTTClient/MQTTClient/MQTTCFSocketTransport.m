...
- (void)open {
    DDLogVerbose(@"[MCMQTTCFSocketTransport] open");
...
+ (NSArray *)clientCertsFromP12:(NSString *)path passphrase:(NSString *)passphrase {
    if (!path) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] no p12 path given");
        return nil;
    }
    
    NSData *pkcs12data = [[NSData alloc] initWithContentsOfFile:path];
    if (!pkcs12data) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] reading p12 failed");
        return nil;
    }
    
    if (!passphrase) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] no passphrase given");
        return nil;
    }
    ...
    if (importStatus != noErr) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] Error while importing pkcs12 [%d]", (int)importStatus);
        return nil;
    }
    
    CFDictionaryRef identityDict = CFArrayGetValueAtIndex(keyref, 0);
    if (!identityDict) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] could not CFArrayGetValueAtIndex");
        return nil;
    }
    ...
    if (!identityRef) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] could not CFDictionaryGetValue");
        return nil;
    };
    ...
    if (status != noErr) {
        DDLogWarn(@"[MCMQTTCFSocketTransport] SecIdentityCopyCertificate failed [%d]", (int)status);
        return nil;
    }
