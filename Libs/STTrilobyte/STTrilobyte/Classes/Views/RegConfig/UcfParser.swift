...
//  trilobyte-lib-ios
//
//  Created by STMicroelectronics MEMS on 13/12/2019.
...
class UcfParser {
    ...
    init(type: VirtualSensorType, regConfig: String, ucfFilename: String, labels: String) {
        ...
        labelMapper = buildRegisterMapperFromString(from: labels) ?? Self.createDefaultValueLabelMapper(type: type)
    }
    ...
    init?(ucf: URL) {
        ...
        for line in lines {
            ...
            } else if (line.contains("<MLC") && line.contains("_SRC>")) {
                if let index = line.firstIndex(of: "<") {
                    mlcLabels.append(String(line[index...]))
                    mlcLabels.append(";")
                }
            } else if (line.contains("<FSM_OUTS") && line.contains(">")) {
                if let index = line.firstIndex(of: "<") {
        ...
        if (steval_supported) {
            ...
            let labelMapperFromFile = buildRegisterMapperFromString(from: labels)
