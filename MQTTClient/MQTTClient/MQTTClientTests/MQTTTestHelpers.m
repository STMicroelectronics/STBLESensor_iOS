...
- (void)sending:(MCMQTTSession *)session
...
- (void)received:(MCMQTTSession *)session
            type:(MQTTCommandType)type
             qos:(MQTTQosLevel)qos
        retained:(BOOL)retained
           duped:(BOOL)duped
...
- (void)subAckReceived:(MCMQTTSession *)session msgID:(UInt16)msgID grantedQoss:(NSArray *)qoss
{
...
+ (NSArray *)clientCerts:(NSDictionary *)parameters {
    ...
    if (parameters[@"clientp12"] && parameters[@"clientp12pass"]) {
        ...
        clientCerts = [MCMQTTCFSocketTransport clientCertsFromP12:path passphrase:parameters[@"clientp12pass"]];
        if (!clientCerts) {
...
+ (MCMQTTSession *)session:(NSDictionary *)parameters {
