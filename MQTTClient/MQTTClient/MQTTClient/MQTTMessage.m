...
@implementation MCMQTTMessage

+ (MCMQTTMessage *)connectMessageWithClientId:(NSString *)clientId
    ...
    if (protocolLevel == MQTTProtocolVersion50) {
        ...
        if (sessionExpiryInterval) {
            [properties appendByte:MCMQTTSessionExpiryInterval];
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTConnect
                                                    data:data];
    return msg;
}

+ (MCMQTTMessage *)pingreqMessage {
    return [[MCMQTTMessage alloc] initWithType:MQTTPingreq];
}

+ (MCMQTTMessage *)disconnectMessage:(MQTTProtocolVersion)protocolLevel
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTDisconnect
                                                    data:data];
    return msg;
}

+ (MCMQTTMessage *)subscribeMessageWithMessageId:(UInt16)msgId
    ...
    MCMQTTMessage* msg = [[MCMQTTMessage alloc] initWithType:MQTTSubscribe
                                                     qos:1
                                                    data:data];
    msg.mid = msgId;
    return msg;
...
+ (MCMQTTMessage *)unsubscribeMessageWithMessageId:(UInt16)msgId
    ...
    MCMQTTMessage* msg = [[MCMQTTMessage alloc] initWithType:MQTTUnsubscribe
                                                     qos:1
                                                    data:data];
    msg.mid = msgId;
    return msg;
...
+ (MCMQTTMessage *)publishMessageWithData:(NSData *)payload
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTPublish
                                                     qos:qosLevel
                                              retainFlag:retain
                                                 dupFlag:dup
                                                    data:data];
...
+ (MCMQTTMessage *)pubackMessageWithMessageId:(UInt16)msgId
                              protocolLevel:(MQTTProtocolVersion)protocolLevel
                                 returnCode:(MQTTReturnCode)returnCode
                               reasonString:(NSString *)reasonString
                               userProperty:(NSDictionary<NSString *,NSString *> *)userProperty {
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTPuback
                                                    data:data];
    msg.mid = msgId;
    return msg;
}

+ (MCMQTTMessage *)pubrecMessageWithMessageId:(UInt16)msgId
                              protocolLevel:(MQTTProtocolVersion)protocolLevel
                                 returnCode:(MQTTReturnCode)returnCode
                               reasonString:(NSString *)reasonString
                               userProperty:(NSDictionary<NSString *,NSString *> *)userProperty {
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTPubrec
                                                    data:data];
    msg.mid = msgId;
    return msg;
}

+ (MCMQTTMessage *)pubrelMessageWithMessageId:(UInt16)msgId
                              protocolLevel:(MQTTProtocolVersion)protocolLevel
                                 returnCode:(MQTTReturnCode)returnCode
                               reasonString:(NSString *)reasonString
                               userProperty:(NSDictionary<NSString *,NSString *> *)userProperty {
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTPubrel
                                                     qos:1
                                                    data:data];
    msg.mid = msgId;
    return msg;
...
+ (MCMQTTMessage *)pubcompMessageWithMessageId:(UInt16)msgId
                               protocolLevel:(MQTTProtocolVersion)protocolLevel
                                  returnCode:(MQTTReturnCode)returnCode
                                reasonString:(NSString *)reasonString
                                userProperty:(NSDictionary<NSString *,NSString *> *)userProperty {
    ...
    MCMQTTMessage *msg = [[MCMQTTMessage alloc] initWithType:MQTTPubcomp
                                                    data:data];
    msg.mid = msgId;
    return msg;
}
...
- (NSData *)wireFormat {
    ...
    DDLogVerbose(@"[MCMQTTMessage] wireFormat(%lu)=%@...",
...
+ (MCMQTTMessage *)messageFromData:(NSData *)data protocolLevel:(MQTTProtocolVersion)protocolLevel {
    MCMQTTMessage *message = nil;
    if (data.length >= 2) {
        UInt8 header;
        ...
        do {
            if (data.length < offset) {
                DDLogWarn(@"[MCMQTTMessage] message data incomplete remaining length");
                offset = -1;
                break;
            }
            ...
            if (multiplier > 128*128*128) {
                DDLogWarn(@"[MCMQTTMessage] message data too long remaining length");
                multiplier = -1;
                break;
            }
        } while ((digit & 0x80) != 0);
        ...
        if (type >= MQTTConnect &&
            ...
            if (offset > 0 &&
                ...
                if ((type == MQTTPublish && (qos >= MQTTQosLevelAtMostOnce && qos <= MQTTQosLevelExactlyOnce)) ||
                    ...
                    message = [[MCMQTTMessage alloc] init];
                    message.type = type;
                    ...
                    if ((type == MQTTPublish &&
                        ...
                        if (message.data.length >= 2) {
                            ...
                            DDLogWarn(@"[MCMQTTMessage] missing packet identifier");
                            message = nil;
                        }
                    }
                    if (type == MQTTPuback ||
                        ...
                        if (protocolLevel != MQTTProtocolVersion50) {
                            if (message.data.length > 2) {
                                DDLogWarn(@"[MCMQTTMessage] unexpected payload after packet identifier");
                                message = nil;
                            }
                        } else {
                            if (message.data.length < 3) {
                                DDLogWarn(@"[MCMQTTMessage] no returncode");
                                message = nil;
                            } else {
                    ...
                    if (type == MQTTPingreq ||
                        ...
                        if (message.data.length > 0) {
                            DDLogWarn(@"[MCMQTTMessage] unexpected payload");
                            message = nil;
                        }
                    }
                    ...
                    if (type == MQTTConnect) {
                        if (message.data.length < 3) {
                            DDLogWarn(@"[MCMQTTMessage] missing connect variable header");
                            message = nil;
                        }
                    }
                    if (type == MQTTConnack) {
                        if (protocolLevel == MQTTProtocolVersion50) {
                            if (message.data.length < 3) {
                                DDLogWarn(@"[MCMQTTMessage] missing connack variable header");
                                message = nil;
                            }
                        } else {
                            if (message.data.length != 2) {
                    ...
                    if (type == MQTTSubscribe) {
                        if (message.data.length < 3) {
                            DDLogWarn(@"[MCMQTTMessage] missing subscribe variable header");
                            message = nil;
                        }
                    }
                    if (type == MQTTSuback) {
                        if (message.data.length < 3) {
                            DDLogWarn(@"[MCMQTTMessage] missing suback variable header");
                            message = nil;
                        }
                    }
                    if (type == MQTTUnsubscribe) {
                        if (message.data.length < 3) {
                            DDLogWarn(@"[MCMQTTMessage] missing unsubscribe variable header");
                            message = nil;
                        }
                    }
                } else {
                    DDLogWarn(@"[MCMQTTMessage] illegal header flags");
                }
            } else {
                DDLogWarn(@"[MCMQTTMessage] remaining data wrong length");
            }
        } else {
            DDLogWarn(@"[MCMQTTMessage] illegal message type");
        }
    } else {
        DDLogWarn(@"[MCMQTTMessage] message data length < 2");
    }
    return message;
}
...
@implementation NSMutableData (MC_MQTT)
