...
/**
 Enumeration of MCMQTTSession states
 */
typedef NS_ENUM(NSInteger, MCMQTTSessionStatus) {
    MCMQTTSessionStatusCreated,
    MCMQTTSessionStatusConnecting,
    MCMQTTSessionStatusConnected,
    MCMQTTSessionStatusDisconnecting,
    MCMQTTSessionStatusClosed,
    MCMQTTSessionStatusError
};

/**
 Enumeration of MCMQTTSession events
 */
typedef NS_ENUM(NSInteger, MCMQTTSessionEvent) {
    MCMQTTSessionEventConnected,
    MCMQTTSessionEventConnectionRefused,
    MCMQTTSessionEventConnectionClosed,
    MCMQTTSessionEventConnectionError,
    MCMQTTSessionEventProtocolError,
    MCMQTTSessionEventConnectionClosedByBroker
};

/**
 The error domain used for all errors created by MCMQTTSession
 */
extern NSString * const MCMQTTSessionErrorDomain;

/**
 The error codes used for all errors created by MCMQTTSession
 */
typedef NS_ENUM(NSInteger, MCMQTTSessionError) {
    MCMQTTSessionErrorConnectionRefused = -8, // Sent if the server closes the connection without sending an appropriate error CONNACK
    MCMQTTSessionErrorIllegalMessageReceived = -7,
    MCMQTTSessionErrorDroppingOutgoingMessage = -6, // For some reason the value is the same as for MCMQTTSessionErrorNoResponse
    MCMQTTSessionErrorNoResponse = -6, // For some reason the value is the same as for MCMQTTSessionErrorDroppingOutgoingMessage
    MCMQTTSessionErrorEncoderNotReady = -5,
    MCMQTTSessionErrorInvalidConnackReceived = -2, // Sent if the message received from server was an invalid connack message
    MCMQTTSessionErrorNoConnackReceived = -1, // Sent if first message received from server was no connack message

    MCMQTTSessionErrorConnackUnacceptableProtocolVersion = 1, // Value as defined by MQTT Protocol
    MCMQTTSessionErrorConnackIdentifierRejected = 2, // Value as defined by MQTT Protocol
    MCMQTTSessionErrorConnackServeUnavailable = 3, // Value as defined by MQTT Protocol
    MCMQTTSessionErrorConnackBadUsernameOrPassword = 4, // Value as defined by MQTT Protocol
    MCMQTTSessionErrorConnackNotAuthorized = 5, // Value as defined by MQTT Protocol
    MCMQTTSessionErrorConnackReserved = 6, // Should be value 6-255, as defined by MQTT Protocol
};

/** Session delegate gives your application control over the MCMQTTSession
 @note all callback methods are optional
 */

@protocol MCMQTTSessionDelegate <NSObject>

@optional

/** gets called when a new message was received
 @param session the MCMQTTSession reporting the new message
 ...
 */
- (void)newMessage:(MCMQTTSession *)session
              data:(NSData *)data
 ...
 */
- (BOOL)newMessageWithFeedback:(MCMQTTSession *)session
                          data:(NSData *)data
                       onTopic:(NSString *)topic
                           qos:(MQTTQosLevel)qos
                      retained:(BOOL)retained
 ...
 */
- (void)session:(MCMQTTSession*)session newMessage:(NSData*)data onTopic:(NSString*)topic;

/** gets called when a connection is established, closed or a problem occurred
 @param session the MCMQTTSession reporting the event
...
/** for mqttio-OBJC backward compatibility
 ...
 @param eventCode the code of the event
 */
- (void)session:(MCMQTTSession*)session handleEvent:(MCMQTTSessionEvent)eventCode;

/** gets called when a connection has been successfully established
 @param session the MCMQTTSession reporting the connect
 
 */
- (void)connected:(MCMQTTSession *)session;
...
/** gets called when a connection has been refused
 @param session the MCMQTTSession reporting the refusal
 @param error an optional additional error object with additional information
 */
- (void)connectionRefused:(MCMQTTSession *)session error:(NSError *)error;

/** gets called when a connection has been closed
 @param session the MCMQTTSession reporting the close

 */
- (void)connectionClosed:(MCMQTTSession *)session;

/** gets called when a connection error happened
 @param session the MCMQTTSession reporting the connect error
 @param error an optional additional error object with additional information
 */
- (void)connectionError:(MCMQTTSession *)session error:(NSError *)error;

/** gets called when an MQTT protocol error happened
 @param session the MCMQTTSession reporting the protocol error
 @param error an optional additional error object with additional information
 */
- (void)protocolError:(MCMQTTSession *)session error:(NSError *)error;

/** gets called when a published message was actually delivered
 @param session the MCMQTTSession reporting the delivery
...
/** gets called when a published message was actually delivered
 ...
 @note this method is called after a publish with qos 1 or 2 only
 */
- (void)messageDelivered:(MCMQTTSession *)session
                   msgID:(UInt16)msgID
...
/** gets called when a subscription is acknowledged by the MQTT broker
 @param session the MCMQTTSession reporting the acknowledge
 ...
 */
- (void)subAckReceived:(MCMQTTSession *)session msgID:(UInt16)msgID grantedQoss:(NSArray<NSNumber *> *)qoss;
 ...
 */
- (void)unsubAckReceived:(MCMQTTSession *)session msgID:(UInt16)msgID;

/** gets called when a command is sent to the MQTT broker
 use this for low level monitoring of the MQTT connection
 @param session the MCMQTTSession reporting the sent command
 ...
 */
- (void)sending:(MCMQTTSession *)session type:(MQTTCommandType)type qos:(MQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;

/** gets called when a command is received from the MQTT broker
 use this for low level monitoring of the MQTT connection
 @param session the MCMQTTSession reporting the received command
 @param type the MQTT command type
 @param qos the Quality of Service of the command
 @param retained the retained status of the command
 @param duped the duplication status of the command
 ...
 @param data the payload data of the command if any, might be zero length
 */
- (void)received:(MCMQTTSession *)session type:(MQTTCommandType)type qos:(MQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;
 ...
 */
- (BOOL)ignoreReceived:(MCMQTTSession *)session type:(MQTTCommandType)type qos:(MQTTQosLevel)qos retained:(BOOL)retained duped:(BOOL)duped mid:(UInt16)mid data:(NSData *)data;
...
/** gets called when the content of MQTTClients internal buffers change
 ...
 @param session the MCMQTTSession reporting the change
 ...
 */
- (void)buffered:(MCMQTTSession *)session
...
@interface MCMQTTSession : NSObject

/** set this member variable to receive delegate messages
 @code
 #import "MQTTClient.h"
 
 @interface MyClass : NSObject <MCMQTTSessionDelegate>
 ...
 @end
 ...
 ...
 - (void)handleEvent:(MCMQTTSession *)session
        event:(MCMQTTSessionEvent)eventCode
        error:(NSError *)error {
    ...
 }
 - (void)newMessage:(MCMQTTSession *)session
 ...
 */
@property (weak, nonatomic) id<MCMQTTSessionDelegate> delegate;
...
/** Control MQTT persistence by setting the properties of persistence before connecting to an MQTT broker.
    ...
    persistence.persistent = YES or NO (default) to establish file or in memory persistence. IMPORTANT: set immediately after creating the MCMQTTSession before calling any other method. Otherwise the default value (NO) will be used
        for this session.
 ...
 */
@property (strong) void (^connectionHandler)(MCMQTTSessionEvent event);
...
/** Session status
 */
@property (nonatomic, readonly) MCMQTTSessionStatus status;
...
/** connects to the specified MQTT server
 ...
 returns nothing and returns immediately. To check the connect results, register as an MCMQTTSessionDelegate and
 - watch for events
...
/** initialises the MQTT session with default values
 @return the initialised MCMQTTSession object
 @code
 #import "MQTTClient.h"
...
/** subscribes to a topic at a specific QoS level
 ...
 @note returns immediately. To check results, register as an MCMQTTSessionDelegate and watch for events.
...
/** closes an MCMQTTSession gracefully
