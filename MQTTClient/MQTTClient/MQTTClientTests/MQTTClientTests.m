...
- (void)test_init_zero_clientId_clean {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        if (self.event == MCMQTTSessionEventConnected) {
            // ok
        } else if (self.event == MCMQTTSessionEventConnectionRefused) {
...
- (void)test_legacy {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        self.session = [[MCMQTTSession alloc] initWithClientId:nil
                                                    userName:nil
                                                    password:nil
                                                   keepAlive:60
                                                cleanSession:TRUE
...
- (void)test_init_zero_clientId_noclean_MQTT_3_1_3_8_MQTT_3_1_3_9 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        XCTAssertEqual(self.event, MCMQTTSessionEventConnectionRefused, @"MCMQTTSessionEventConnectionRefused %@", self.error);
        XCTAssert(self.error.code == 0x02, @"error = %@", self.error);
        [self shutdown:parameters];
    }
}
...
- (void)test_connect_will_non_retained_MQTT_3_1_2_8_MQTT_3_1_2_16 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        MCMQTTSession *subscribingSession = [MQTTTestHelpers session:parameters];
...
- (void)test_connect_will_unflagged_but_retain_not_0_MQTT_3_1_2_15 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        XCTAssertEqual(self.event, MCMQTTSessionEventConnectionClosedByBroker,
...
- (void)test_disconnect_when_same_clientID_connects_MQTT_3_1_4_2 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        MCMQTTSession *sameSession = [MQTTTestHelpers session:parameters];
...
- (void)test_connect_protocollevel3__MQTT_3_1_2_1 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        XCTAssertEqual(self.event, MCMQTTSessionEventConnected, @"No MCMQTTSessionEventConnected %@", self.error);
...
- (void)test_connect_protocollevel5__MQTT_3_1_2_1 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        XCTAssertEqual(self.event, MCMQTTSessionEventConnectionClosedByBroker, @"session not closed %@", self.error);
...
- (void)test_connect_illegal_protocollevel88_MQTT_3_1_2_2_MQTT_3_2_2_5 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        XCTAssert(!self.timedout, @"timeout");
        XCTAssert(self.connectionError.code == MCMQTTSessionErrorConnackUnacceptableProtocolVersion, @"error = %@", self.connectionError);
...
- (void)test_connect_illegal_protocollevel0_and_protocolname_MQTT_3_1_2_1 {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        if (self.event == MCMQTTSessionEventConnectionClosedByBroker ||
            self.event == MCMQTTSessionEventConnectionError ||
            (self.event == MCMQTTSessionEventConnectionRefused && self.error && self.error.code == 0x01)) {
...
- (void)test_ping {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        XCTAssertNotEqual(self.event, MCMQTTSessionEventConnectionClosed, @"MCMQTTSessionEventConnectionClosed %@", self.error);
        XCTAssertNotEqual(self.event, MCMQTTSessionEventProtocolError, @"MCMQTTSessionEventProtocolError %@", self.error);
...
- (void)test_systopic {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        [self connect:parameters];
        XCTAssert(!self.timedout, @"timeout");
        XCTAssertEqual(self.event, MCMQTTSessionEventConnected, @"MCMQTTSessionEventConnected %@", self.error);
...
- (void)no_cleansession:(MQTTQosLevel)qos {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        MCMQTTSession *sendingSession = [MQTTTestHelpers session:parameters];
