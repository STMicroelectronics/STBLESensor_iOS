...
NSString * const MCMQTTSessionErrorDomain = @"MQTT";

@interface MCMQTTSession() <MCMQTTDecoderDelegate, MQTTTransportDelegate>
...
- (instancetype)init {
    DDLogVerbose(@"[MCMQTTSession] init");
    self = [super init];
    self.txMsgId = 1;
...
- (UInt16)subscribeToTopics:(NSDictionary<NSString *, NSNumber *> *)topics subscribeHandler:(MQTTSubscribeHandler)subscribeHandler {
    DDLogVerbose(@"[MCMQTTSession] subscribeToTopics:%@]", topics);
    ...
    (void)[self encode:[MCMQTTMessage subscribeMessageWithMessageId:mid
                                                           topics:topics
...
- (UInt16)unsubscribeTopics:(NSArray<NSString *> *)topics unsubscribeHandler:(MQTTUnsubscribeHandler)unsubscribeHandler {
    DDLogVerbose(@"[MCMQTTSession] unsubscribeTopics:%@", topics);
    ...
    (void)[self encode:[MCMQTTMessage unsubscribeMessageWithMessageId:mid
                                                             topics:topics
                                                      protocolLevel:self.protocolLevel]];
    return mid;
}
...
{
    DDLogVerbose(@"[MCMQTTSession] publishData:%@... onTopic:%@ retain:%d qos:%ld publishHandler:%p",
    ...
    UInt16 msgId = 0;
    if (!qos) {
        MCMQTTMessage *msg = [MCMQTTMessage publishMessageWithData:data
                                                       onTopic:topic
                                                           qos:qos
                                                         msgId:msgId
        ...
        NSError *error = nil;
        if (![self encode:msg]) {
            error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                        code:MCMQTTSessionErrorEncoderNotReady
        ...
        MCMQTTMessage *msg = nil;

        id<MQTTFlow> flow;
        if (self.status == MCMQTTSessionStatusConnected) {
            ...
            if (unprocessedMessageNotExists && windowSize <= self.persistence.maxWindowSize) {
                msg = [MCMQTTMessage publishMessageWithData:data
                                                  onTopic:topic
                                                      qos:qos
                                                    msgId:msgId
                                               retainFlag:retainFlag
        ...
        if (!flow) {
            DDLogWarn(@"[MCMQTTSession] dropping outgoing message %d", msgId);
            NSError *error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                                 code:MCMQTTSessionErrorDroppingOutgoingMessage
            ...
            if ((flow.commandType).intValue == MQTTPublish) {
                DDLogVerbose(@"[MCMQTTSession] PUBLISH %d", msgId);
                if (![self encode:msg]) {
                    DDLogInfo(@"[MCMQTTSession] queueing message %d after unsuccessfull attempt", msgId);
                    flow.commandType = [NSNumber numberWithUnsignedInt:MQTT_None];
                    flow.deadline = [NSDate date];
                    [self.persistence sync];
                }
            } else {
                DDLogInfo(@"[MCMQTTSession] queueing message %d", msgId);
            }
        }
    }
    [self tell];
...
- (void)closeWithReturnCode:(MQTTReturnCode)returnCode
    ...
    DDLogVerbose(@"[MCMQTTSession] closeWithDisconnectHandler:%p ", disconnectHandler);
...
- (void)closeInternal
{
    ...
    self.status = MCMQTTSessionStatusClosed;
    if ([self.delegate respondsToSelector:@selector(handleEvent:event:error:)]) {
        [self.delegate handleEvent:self event:MCMQTTSessionEventConnectionClosed error:nil];
    }
    ...
    NSError *error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                         code:MCMQTTSessionErrorNoResponse
...
- (void)checkTxFlows {
    ...
    for (id<MQTTFlow> flow in flows) {
        DDLogVerbose(@"[MCMQTTSession] %@ flow %@ %@ %@", self.clientId, flow.deadline, flow.commandType, flow.messageId);
        if ([flow.deadline compare:[NSDate date]] == NSOrderedAscending) {
            switch ((flow.commandType).intValue) {
                case 0:
                    if (windowSize <= self.persistence.maxWindowSize) {
                        DDLogVerbose(@"[MCMQTTSession] PUBLISH queued message %@", flow.messageId);
                        message = [MCMQTTMessage publishMessageWithData:flow.data
                                                              onTopic:flow.topic
                    ...
                    break;
                case MQTTPublish:
                    DDLogInfo(@"[MCMQTTSession] resend PUBLISH %@", flow.messageId);
                    message = [MCMQTTMessage publishMessageWithData:flow.data
                                                          onTopic:flow.topic
                                                              qos:(flow.qosLevel).intValue
                                                            msgId:(flow.messageId).intValue
                    ...
                    break;
                case MQTTPubrel:
                    DDLogInfo(@"[MCMQTTSession] resend PUBREL %@", flow.messageId);
                    message = [MCMQTTMessage pubrelMessageWithMessageId:(flow.messageId).intValue
                                                        protocolLevel:self.protocolLevel
...
- (void)decoder:(MCMQTTDecoder*)sender handleEvent:(MCMQTTDecoderEvent)eventCode error:(NSError *)error {
    ...
    DDLogVerbose(@"[MCMQTTSession] decoder handleEvent: %@ (%d) %@",
                 events[eventCode % [events count]],
                 eventCode,
                 [error description]);

    switch (eventCode) {
        case MCMQTTDecoderEventConnectionClosed:
            [self error:MCMQTTSessionEventConnectionClosedByBroker error:error];
            break;
        case MCMQTTDecoderEventConnectionError:
            [self connectionError:error];
            break;
        case MCMQTTDecoderEventProtocolError:
...
- (void)decoder:(MCMQTTDecoder*)sender didReceiveMessage:(NSData *)data {
    ...
    if (!message) {
        ...
        NSError * error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                              code:MCMQTTSessionErrorIllegalMessageReceived
    ...
    @synchronized(sender) {
        ...
        switch (self.status) {
            case MCMQTTSessionStatusConnecting:
                switch (message.type) {
                    case MQTTConnack:
                        if ((self.protocolLevel == MQTTProtocolVersion50 && message.data.length < 3) ||
                            ...
                            NSError *error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                                                 code:MCMQTTSessionErrorInvalidConnackReceived
                            ...
                            if (message.returnCode && (message.returnCode).intValue == MQTTSuccess) {
                                self.status = MCMQTTSessionStatusConnected;
                                ...
                                if ([self.delegate respondsToSelector:@selector(handleEvent:event:error:)]) {
                                    [self.delegate handleEvent:self event:MCMQTTSessionEventConnected error:nil];
                                }
                                ...
                                if (message.returnCode) {
                                    switch ((message.returnCode).intValue) {
                                        case 1:
                                            ...
                                            errorCode = MCMQTTSessionErrorConnackUnacceptableProtocolVersion;
                                            break;
                                        case 2:
                                            errorDescription = @"MQTT CONNACK: identifier rejected";
                                            errorCode = MCMQTTSessionErrorConnackIdentifierRejected;
                                            break;
                                        case 3:
                                            errorDescription = @"MQTT CONNACK: server unavailable";
                                            errorCode = MCMQTTSessionErrorConnackServeUnavailable;
                                            break;
                                        case 4:
                                            errorDescription = @"MQTT CONNACK: bad user name or password";
                                            errorCode = MCMQTTSessionErrorConnackBadUsernameOrPassword;
                                            break;
                                        case 5:
                                            errorDescription = @"MQTT CONNACK: not authorized";
                                            errorCode = MCMQTTSessionErrorConnackNotAuthorized;
                                            break;
                                        default:
                                            errorDescription = @"MQTT CONNACK: reserved for future use";
                                            errorCode = MCMQTTSessionErrorConnackReserved;
                                            break;
                                    }
                                }
                                ...
                                [self error:MCMQTTSessionEventConnectionRefused error:error];
                    ...
                    default: {
                        NSError * error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                                              code:MCMQTTSessionErrorNoConnackReceived
                ...
                break;
            case MCMQTTSessionStatusConnected:
                switch (message.type) {
                    case MQTTPublish:
                ...
                break;
            default:
                DDLogError(@"[MCMQTTSession] other state");
                break;
        }
    }
}

- (void)handlePublish:(MCMQTTMessage*)msg {
    ...
    if (msg.qos == 0) {
        ...
        if (data.length >= 2) {
            ...
            if (msg.qos == 1) {
                ...
                if (processed) {
                    (void)[self encode:[MCMQTTMessage pubackMessageWithMessageId:msgId
                                                                 protocolLevel:self.protocolLevel
                                                                    returnCode:MQTTSuccess
                                                                  reasonString:nil
                                                                  userProperty:nil]];
                ...
                if (![self.persistence storeMessageForClientId:self.clientId
                                                  ...
                                                  incomingFlag:YES
                                                   commandType:MQTTPubrec
                                                      deadline:[NSDate dateWithTimeIntervalSinceNow:self.dupTimeout]]) {
                    DDLogWarn(@"[MCMQTTSession] dropping incoming messages");
                } else {
                    [self.persistence sync];
                    [self tell];
                    (void)[self encode:[MCMQTTMessage pubrecMessageWithMessageId:msgId
                                                                 protocolLevel:self.protocolLevel
                                                                    returnCode:MQTTSuccess
                                                                  reasonString:nil
                                                                  userProperty:nil]];
...
- (void)handlePuback:(MCMQTTMessage*)msg {
...
- (void)handleSuback:(MCMQTTMessage*)msg
{
...
- (void)handleUnsuback:(MCMQTTMessage *)message {
...
- (void)handlePubrec:(MCMQTTMessage *)message {
    MCMQTTMessage *pubrelmessage = [MCMQTTMessage pubrelMessageWithMessageId:message.mid
                                                           protocolLevel:self.protocolLevel
                                                              returnCode:MQTTSuccess
                                                            reasonString:nil
                                                            userProperty:nil];
...
- (void)handlePubrel:(MCMQTTMessage *)message {
    id<MQTTFlow> flow = [self.persistence flowforClientId:self.clientId
                                             incomingFlag:YES
                                                messageId:message.mid];
    if (flow) {
        ...
        if (processed) {
            ...
            [self.persistence sync];
            [self tell];
            (void)[self encode:[MCMQTTMessage pubcompMessageWithMessageId:message.mid
                                                          protocolLevel:self.protocolLevel
                                                             returnCode:MQTTSuccess
                                                           reasonString:nil
                                                           userProperty:nil]];
...
- (void)handlePubcomp:(MCMQTTMessage *)message {
    id<MQTTFlow> flow = [self.persistence flowforClientId:self.clientId
                                             incomingFlag:NO
                                                messageId:message.mid];
...
- (void)connectionError:(NSError *)error {
    [self error:MCMQTTSessionEventConnectionError error:error];
...
- (void)protocolError:(NSError *)error {
    [self error:MCMQTTSessionEventProtocolError error:error];
    if ([self.delegate respondsToSelector:@selector(protocolError:error:)]) {
        [self.delegate protocolError:self error:error];
    }
}

- (void)error:(MCMQTTSessionEvent)eventCode error:(NSError *)error {
    self.status = MCMQTTSessionStatusError;
    if ([self.delegate respondsToSelector:@selector(handleEvent:event:error:)]) {
    ...
    if(eventCode == MCMQTTSessionEventConnectionClosedByBroker && self.connectHandler) {
        error = [NSError errorWithDomain:MCMQTTSessionErrorDomain
                                    code:MCMQTTSessionErrorConnectionRefused
...
- (void)connect {
    ...
    DDLogVerbose(@"[MCMQTTSession] connecting");
...
- (void)connectWithConnectHandler:(MQTTConnectHandler)connectHandler {
    DDLogVerbose(@"[MCMQTTSession] connectWithConnectHandler:%p", connectHandler);
    self.connectHandler = connectHandler;
    [self connect];
}

- (BOOL)encode:(MCMQTTMessage *)message {
    if (message) {
        ...
        if (wireFormat) {
            ...
            DDLogVerbose(@"[MCMQTTSession] mqttTransport send");
            return [self.transport send:wireFormat];
        } else {
            DDLogError(@"[MCMQTTSession] trying to send message without wire format");
            return false;
        }
    } else {
        DDLogError(@"[MCMQTTSession] trying to send nil message");
        return false;
    }
}
...
- (void)mqttTransport:(id<MQTTTransport>)mqttTransport didReceiveMessage:(NSData *)message {
    DDLogVerbose(@"[MCMQTTSession] mqttTransport didReceiveMessage");
...
- (void)mqttTransportDidClose:(id<MQTTTransport>)mqttTransport {
    DDLogVerbose(@"[MCMQTTSession] mqttTransport mqttTransportDidClose");

    [self error:MCMQTTSessionEventConnectionClosedByBroker error:nil];

}

- (void)mqttTransportDidOpen:(id<MQTTTransport>)mqttTransport {
    DDLogVerbose(@"[MCMQTTSession] mqttTransportDidOpen");

    DDLogVerbose(@"[MCMQTTSession] sending CONNECT");

    if (!self.connectMessage) {
        (void)[self encode:[MCMQTTMessage connectMessageWithClientId:self.clientId
...
- (void)mqttTransport:(id<MQTTTransport>)mqttTransport didFailWithError:(NSError *)error {
    DDLogWarn(@"[MCMQTTSession] mqttTransport didFailWithError %@", error);
