...
- (void)testWSTRANSPORT {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        if ([parameters[@"websocket"] boolValue]) {
            ...
            XCTAssert(!self.timedout, @"timeout");
            XCTAssertEqual(self.event, MCMQTTSessionEventConnectionClosedByBroker, @"Not ClosedByBroker %ld %@", (long)self.event, self.error);
        }
    }
...
- (void)testWSLowLevel {
    for (NSString *broker in self.brokers.allKeys) {
        ...
        if ([parameters[@"websocket"] boolValue]) {
            ...
            MCMQTTMessage *connectMessage = [MCMQTTMessage connectMessageWithClientId:@"SRWebsocket"
                                                                         userName:nil
                                                                         password:nil
                                                                        keepAlive:10
                                                                     cleanSession:true
...
- (void)connect:(MCMQTTSession *)session parameters:(NSDictionary *)parameters{
    self.session.delegate = self;
    self.session.userName = parameters[@"user"];
    self.session.password = parameters[@"pass"];
