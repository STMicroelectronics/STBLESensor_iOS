...
- (void)testPublishCloseExpected:(NSData *)data onTopic:(NSString *)topic retain:(BOOL)retain atLevel:(UInt8)qos
{
    ...
    XCTAssert(
              (self.event == MCMQTTSessionEventConnectionClosedByBroker) ||
              (self.event == MCMQTTSessionEventConnectionError),
              @"No MCMQTTSessionEventConnectionClosedByBroker or MCMQTTSessionEventConnectionError happened");
}
...
- (void)testPublish:(NSData *)data onTopic:(NSString *)topic retain:(BOOL)retain atLevel:(UInt8)qos
{
    ...
    switch (qos % 4) {
        ...
        case 3:
        default:
            XCTAssert(self.event == (long)MCMQTTSessionEventConnectionClosed, @"no close received");
            break;
    }
}
...
- (void)newMessage:(MCMQTTSession *)session data:(NSData *)data onTopic:(NSString *)topic
